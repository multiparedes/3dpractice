<template>
  <div
    class="flex fixed flex-col bottom-10 left-4 py-4 px-2 items-center gap-4 bg-[#F8F8F8]/60 dark:bg-slate-900/80 rounded-2xl"
  >
    <ul v-for="section in sections" :key="section.name">
      <MyTooltip :text="section.name">
        <Icon
          :icon="section.icon"
          class="h-6 w-6 text-slate-950 dark:text-white dark:hover:text-secondary-400 hover:text-primary-400 transition-colors"
